...
polyOrder = np.array([5])
#polyOrder = np.array([1])#To test the code
MC_sims_no = np.array([10,100,1000,10000,48000,1e5,5e5,1e6,1e7,1e8,288e6,500e6,1e9])
...
poly_load_thrust = cp.load('BEM_Thrust_SM_PolyOrder_5.npz',allow_pickle=True)
poly_load_torque = cp.load('BEM_Torque_SM_PolyOrder_5.npz',allow_pickle=True)
...
f=open(tstr_file+'_CPU_Time_P5_MC_trt.pckl','wb')
pickle.dump(cpu_time_trt,f)
f.close()

f=open(tstr_file+'_CPU_Time_P5_MC_trq.pckl','wb')
