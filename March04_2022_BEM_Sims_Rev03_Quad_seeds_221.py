...
distribution = cp.Iid(cp.Uniform(0, 1), 10)
sparse_quads = [
    cp.generate_quadrature(order, distribution,
                           rule="legendre") for order in range(2,6)]
#%%
NoOfRlz = np.zeros(4)

for i in range(0,4):
    NoOfRlz[i] = int(sparse_quads[i][1].shape[0])

NoOfRlz = NoOfRlz[0:1]

NoOfRlz = NoOfRlz.astype(int)
#%%
kk= 0

for Seeds in NoOfRlz:
    xi = sparse_quads[kk][0]
    w = sparse_quads[kk][1]
    ...
    pickle.dump(xi,f)
    f.close()
    file_path_w = newpath+'/w_'+str(Seeds)+'seeds.pckl'
    f = open(file_path_w ,'wb')
    pickle.dump(w,f)
    f.close()
    for i in range(9,10):#range(0,len(wind_speeds)):        
