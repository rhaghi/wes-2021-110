...
NoOfRlz = np.array([1581]) #This is based on the polynomial order and the number of random variables
...
for i in range(0, NoOfRlz.size):
    ...
    file_path_w =  'w_' + str(NoOfRlz[i].astype(int))+'seeds.pckl'
    ...
    for j in range(ITStep, SimLength[i].astype(int)):
        ...
        solves_trt = np.zeros(NoOfRlz[i].astype(int))
        solves_trt = Thrust[:, j]
        solves_trq = np.zeros(NoOfRlz[i].astype(int))
        solves_trq = Torque[:, j]
        t0 = time.time()
        #approx_trt  = cp.fit_regression(orthPoly, dataPoints, samples_trt)
        approx_trt  = cp.fit_quadrature(orthPoly, xi, w, solves_trt)
        cpu_time_loop[k,0] = time.time()-t0
        t0 = time.time()
        #approx_trq = cp.fit_regression(orthPoly, dataPoints, samples_trq)
        approx_trq  = cp.fit_quadrature(orthPoly, xi, w, solves_trq)
