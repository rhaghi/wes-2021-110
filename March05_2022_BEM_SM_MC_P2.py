...
SimLength = np.array([16])#,10,10,10])
...
MC_sims_no = np.array([10,100,1000,10000,48000,1e5,5e5,1e6,1e7])#,1e8,288e6,500e6])
...
poly_load_thrust = cp.load('BEM_Thrust_SM_PolyOrder_2.npz',allow_pickle=True)
poly_load_torque = cp.load('BEM_Torque_SM_PolyOrder_2.npz',allow_pickle=True)
...
f=open(tstr_file+'_CPU_Time_P2_MC_trt.pckl','wb')
pickle.dump(cpu_time_trt,f)
f.close()

f=open(tstr_file+'_CPU_Time_P2_MC_trq.pckl','wb')
